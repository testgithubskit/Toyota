  ...
  },
  {
    ...
    customRender: ({ record }) => {
      return h('div', [
        ...
        }, [h(LineChartOutlined), 'View History']),
        h(Button, {
...
const handleViewHistory = (record) => {
  ...
  const now = dayjs();
  const sevenDaysAgo = now.subtract(5, 'year');
  historyDateRange.value = [sevenDaysAgo, now];
  fetchAndRenderHistoryChart();
};

const fetchAndRenderHistoryChart = async () => {
  if (!selectedComparison.value || !historyDateRange.value || historyDateRange.value.length !== 2) {
...
const renderHistoryChart = () => {
  ...
  if (CompareParameters.formattedHistoryData.length === 0) {
  ...
  const option = {
    ...
    legend: {
      data: [selectedComparison.value.machine_parameter1_name, selectedComparison.value.machine_parameter2_name],
      bottom: 0
    },
    grid: {
      left: '3%',
    ...
    series: [
      {
        name: selectedComparison.value.machine_parameter1_name,
        type: 'line',
        data: CompareParameters.formattedHistoryData.map(item => [new Date(item.time).getTime(), item.value_1])
      },
      {
        name: selectedComparison.value.machine_parameter2_name,
        type: 'line',
        data: CompareParameters.formattedHistoryData.map(item => [new Date(item.time).getTime(), item.value_2])
      }
    ]
  };
          ...
          <div class="w-1/3 pl-4">
            <CardBox class="h-full">
              <div class="flex justify-between items-center mb-4">
                <h2 class="text-xl font-semibold">{{ selectedComparison ? selectedComparison.machine_name : 'No' }} History</h2>
                <Button v-if="selectedComparison" @click="selectedComparison = null" icon="close">
                  <template #icon><CloseOutlined /></template>
                </Button>
              </div>
              <div v-if="selectedComparison" class="mb-4">
                <div class="mb-2">
                ...
                <div class="mb-4">
                  <Button 
                    ...
                    :disabled="!historyDateRange || historyDateRange.length !== 2 || isLoading"
                    class="w-full"
                  >
              ...
              <div v-else class="text-center text-gray-500">
                Select a comparison to view its history
              </div>
