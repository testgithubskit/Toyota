  ...
  },
  {
    ...
    customRender: ({ record }) => {
      return h('div', [
        ...
        h(Button, {
          onClick: () => editableRows.value[record.id] ? handleSave(record) : handleEdit(record),
          type: editableRows.value[record.id] ? 'success' : 'default',
          size: 'small',
          icon: h(editableRows.value[record.id] ? SaveOutlined : EditOutlined),
          style: { marginRight: '8px', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' },
          class: editableRows.value[record.id] 
            ? "border border-green-500 text-green-500 hover:bg-green-50"
            : "border border-blue-500 text-blue-500 hover:bg-blue-50",
        }, editableRows.value[record.id] ? '' : ''),
        h(Popconfirm, {
...
const startAutoRefresh = () => {
  ...
  }, 30000); // Refresh every 50 seconds
};
...
const handleViewHistory = (record) => {
  ...
  console.log("Selected Comparison")
  console.log(selectedComparison.value)
};
...
const isDateRangeValid = computed(() => {
  ...
  const isValid = end.isAfter(start) && end.diff(start, 'days') <= 7;
  console.log('isDateRangeValid:', isValid, 'startDate:', startDate.value, 'endDate:', endDate.value);
  return isValid;
});

const maxStartDate = computed(() => dayjs().format('YYYY-MM-DDTHH:mm'));
const minEndDate = computed(() => startDate.value ? dayjs(startDate.value).add(1, 'minute').format('YYYY-MM-DDTHH:mm') : '');
const maxEndDate = computed(() => {
  if (!startDate.value) return dayjs().format('YYYY-MM-DDTHH:mm');
  const maxDate = dayjs(startDate.value).add(7, 'days');
  return dayjs(maxDate).isBefore(dayjs()) ? maxDate.format('YYYY-MM-DDTHH:mm') : dayjs().format('YYYY-MM-DDTHH:mm');
});
...
const validateDateRange = () => {
  ...
  if (startDate.value && endDate.value) {
    if (!isDateRangeValid.value) {
      message.error('Please select a valid date range (up to 7 days).');
      endDate.value = ''; // Reset only the end date
    }
  }
};

const fetchAndRenderHistoryChart = async () => {
  console.log('fetchAndRenderHistoryChart called');
  console.log('selectedComparison:', selectedComparison.value);
  console.log('startDate:', startDate.value);
  console.log('endDate:', endDate.value);
  console.log('isDateRangeValid:', isDateRangeValid.value);

  if (!selectedComparison.value || !isDateRangeValid.value) {
  ...
  const formData = {
    ...
    fromDate: dayjs(startDate.value).format('YYYY-MM-DD HH:mm:ss'),
    toDate: dayjs(endDate.value).format('YYYY-MM-DD HH:mm:ss'),
  };
...
const graphType = ref('line'); // New state for graph type
const showDataPoints = ref(false); // New state for showing data points
    ...
    series: [
      {
        ...
        type: graphType.value, // Use selected graph type
        data: parameter1Data.map(item => [item.time, item.value]),
        symbol: showDataPoints.value ? 'circle' : 'none' // Show data points based on user selection
      },
      {
...
const resetGraph = () => {
  startDate.value = '';
  endDate.value = '';
  graphType.value = 'line'; // Reset to default graph type
  showDataPoints.value = false; // Reset data points visibility
  renderHistoryChart(); // Re-render the chart
};

const downloadGraphData = () => {
  const dataToDownload = CompareParameters.formattedHistoryData; // Adjust this to your data structure
  const blob = new Blob([JSON.stringify(dataToDownload)], { type: 'application/json' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = 'graph_data.json'; // Change the file name as needed
...
// Add a watch for startDate and endDate
watch([startDate, endDate], () => {
  console.log('Date changed - startDate:', startDate.value, 'endDate:', endDate.value);
});
</script>
  ...
  <LayoutAuthenticatedSimple>
    <SectionMain>
      <div class="container mx-auto px-4 py-8">
        ...
        <div v-if="selectedComparison" class="max-w-8xl mx-auto p-6 bg-white rounded-lg shadow-lg">
    <h2 class="text-2xl font-semibold text-emerald-700 mb-6">{{ selectedComparison.machine_name }} History</h2>
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
      <div class="bg-gray-50 p-4 rounded-md">
        <span class="font-medium text-gray-700">Parameter 1:</span> 
        <span class="ml-2 text-emerald-600">{{ selectedComparison.machine_parameter1_name }}</span>
      </div>
      <div class="bg-gray-50 p-4 rounded-md">
        <span class="font-medium text-gray-700">Parameter 2:</span> 
        <span class="ml-2 text-emerald-600">{{ selectedComparison.machine_parameter2_name }}</span>
      </div>
    </div>
    ...
    <div class="mb-6">
      <div class="flex justify-between items-center mb-4">
        <div class="flex items-center space-x-4">
          <label for="graphType" class="font-medium text-gray-700">Graph Type:</label>
          <select 
            id="graphType"
            v-model="graphType" 
        ...
        <div class="space-x-4">
          <button @click="resetGraph" class="px-4 py-2 bg-gray-200 hover:bg-gray-300 rounded-md transition duration-150 ease-in-out">
            Reset
          </button>
          <button @click="downloadGraphData" class="px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-md transition duration-150 ease-in-out">
            Download Data
          </button>
        </div>
      </div>
      ...
      <div class="flex space-x-6">
        <div class="w-1/2">
          <label for="startDate" class="block mb-2 font-medium text-gray-700">Start Date and Time</label>
          <input 
  type="datetime-local" 
  id="startDate" 
  v-model="startDate" 
...
>
        ...
        <div class="w-1/2">
          <label for="endDate" class="block mb-2 font-medium text-gray-700">End Date and Time</label>
          <input 
  type="datetime-local" 
  id="endDate" 
  v-model="endDate" 
...
>
    ...
    <div class="flex justify-end mb-6">
      <button
        ...
        class="px-6 py-3 bg-emerald-500 hover:bg-emerald-600 text-white rounded-md transition duration-150 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed"
      >
    ...
    <div
      ref="historyChart"
      class="w-full h-[500px] border border-gray-200 rounded-lg shadow-lg bg-white"
    ></div>
  </div>
