...
const validateDateRange = () => {
  if (historyDateRange.value && historyDateRange.value.length === 2) {
    const startDate = historyDateRange.value[0];
    const endDate = historyDateRange.value[1];
    const diffDays = endDate.diff(startDate, 'days');
    if (diffDays > 7) {
      message.error('Date range cannot exceed one week.');
      historyDateRange.value = []; // Reset the date range
    }
  }
};
  ...
  <LayoutAuthenticatedSimple>
      ...
      <div class="md:col-span-2">
        <div class="flex justify-between mb-4">
          <div class="flex items-center">
            <select v-model="graphType" class="border rounded px-2 py-1">
              <option value="line">Line</option>
              <option value="bar">Bar</option>
            </select>
          </div>
          <div>
            <button @click="resetGraph" class="mr-2 px-4 py-2 bg-gray-200 rounded">Reset</button>
            <button @click="downloadGraphData" class="px-4 py-2 bg-blue-500 text-white rounded">Download Data</button>
          </div>
        </div>
    ...
    <div class="flex justify-end mb-4">
      <button
        ...
        class="px-4 py-2 bg-emerald-500 text-white rounded disabled:opacity-50"
      >
