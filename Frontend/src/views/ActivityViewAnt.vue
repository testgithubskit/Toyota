...
import { Table, Input, DatePicker, Button, Switch, InputNumber, Select, Space } from 'ant-design-vue';
...
const handleQuerySubmit = async () => {
  try {
    ...
    pendingData.value = ActivityStoredata.pendingData.map(item => ({ ...item, key: item.id }));
    completedData.value = ActivityStoredata.completedData.map(item => ({ ...item, key: item.id }));
  ...
  refreshTimer = setInterval(() => {
    switch (currentFilter.value) {
      case 'BLOCK':
        ...
        applyFilter('location', 'like', 'CRANK');
        break;
      case 'CRANK':
        currentFilter.value = 'HEAD';
        applyFilter('location', 'like', 'HEAD');
        break;
      case 'HEAD': 
        currentFilter.value = 'BLOCK'; 
        applyFilter('location', 'like', 'BLOCK');
        break;
      default: 
        currentFilter.value = 'BLOCK'; 
        applyFilter('location', 'like', 'BLOCK');
...
const applyFilter = () => {
  if (filterField.value && filterValue.value) {
    pendingData.value = ActivityStoredata.pendingData.filter(item => {
      const fieldValue = item[filterField.value];
      if (filterType.value === 'like') {
        return fieldValue && fieldValue.toLowerCase().includes(filterValue.value.toLowerCase());
      } else {
    ...
    <SectionMain>
      <div class="container mx-auto flex flex-col space-y-4">
        ...
        <div class="auto-refresh-container p-4 mt-4 flex items-center space-x-4">
          ...
          <div v-if="autoRefresh" class="flex items-center space-x-2">
            ...
            <a-input-number
              ...
              style="width: 60px"
            />
            <a-button type="primary" @click="startAutoRefresh">Set Interval</a-button>
          </div>
        ...
        <div class="w-full flex justify-normal space-x-4 mb-4">
          <a-space>
            <a-select v-model:value="filterField" style="width: 120px">
              <a-select-option value="name">Name</a-select-option>
              <a-select-option value="location">Location</a-select-option>
              <a-select-option value="group_name">Group Name</a-select-option>
              <a-select-option value="parameter_name">Parameter Name</a-select-option>
            </a-select>
            ...
            <a-input v-model:value="filterValue" placeholder="Filter value" style="width: 200px" />
            <a-button type="primary" @click="applyFilter">Apply Filter</a-button>
            <a-button @click="clearFilter">Clear Filter</a-button>
          </a-space>
