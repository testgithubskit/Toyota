...
const saveEdit = async () => {
  ...
  try {
    ...
    // Check if any row has been changed to "Completed" status without filling mandatory fields
    ...
    if (incompleteRows.length > 0) {
      ...
      Toastify({
        ...
        duration: 5000, // Extend duration for better visibility
        close: true,
        gravity: 'bottom',
        position: 'right',
        backgroundColor: 'red',
    ...
    // Update data before sending for editing
    editedRowsData.forEach(row => {
      // Update any null values (other than target_date_of_completion and responsible_person_company_id) with "-"
...
const exportData = () => {
  ...
  let tableData3 = []
    ...
    if (tableData.value && Array.isArray(tableData.value)) {
      let tableData3 = tableData.value;
  } else if (!Array.isArray(tableData3)) {
    console.error('Data is not an array:', tableData3);
    return;
  }

    // Convert tableData to Excel format
    let data = tableData3.map(item => [
      item.slno,
      item.line,
      item.criticalWarning,
      item.opNo,
      item.problemsIdentified,
      item.identificationDate,
      item.counterMeasurement,
      item.spareReq,
    ...
    // Add header row
    data.unshift(['Sl. No.', 'Line', 'Critical Warning', 'Op No.', 'Problems Identified', 'Identification Date', 'Counter Measurement', 'Spare Req', 'Support Required', 'Pending Days', 'Trg Date', 'Completion Date', 'Resp', 'Status']);
  ...
  let tableData4 = []
    const fileName = 'data.xlsx';
    if (tableData2.value && Array.isArray(tableData2.value)) {
      let tableData3 = tableData2.value;
  } else if (!Array.isArray(tableData4)) {
    console.error('Data is not an array:', tableData4);
    return;
  }

    // Convert tableData to Excel format
    let data = tableData4.map(item => [
      item.slno,
      item.line,
      item.criticalWarning,
      item.opNo,
  ...
  <LayoutAuthenticatedSimple class=" font-custom ">
    ...
    <BlurryHorizontalDivider class="mt-4" />
        ...
        <div class="w-full flex justify-normal space-x-4" v-if="!autoRefresh">
          ...
          <div class="w-2/4  flex justify-end h-18 items-end">
            <BaseButton type="button" color="success" label="Export data" @click="exportData" />
          </div>
        </div>
        ...
        <div>
          <div class="w-2/4 flex justify-center  mt-8 mb-4">
            ...
            <div class="w-2/4  flex justify-end h-18 items-end">
            <BaseButton type="button" color="success" label="Export data" @click="exportData2" />
          </div>
          </div>
