  ...
  <div class="relative w-full md:w-64 lg:w-80">
    ...
    >
      <div class="flex justify-between items-center h-6">
        <span class="mr-2">{{ selectedItem ? selectedItem.label : 'Select a machine' }}</span>
      </div>
    </div>
    <div
      v-if="isDropdownOpen"
      class="absolute top-full left-0 w-full border rounded shadow bg-white z-10 mt-2 whitespace-normal"
    >
      <div class="p-2 max-h-48 overflow-y-auto m-2">
        ...
        />
        <div v-for="(item, index) in filteredItems" :key="index" class="whitespace-normal">
