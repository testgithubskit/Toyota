...
export const useCompareParameters = defineStore('CompareParameters', {
  ...
  getters: {
    parameterGroups() {
      return [
        // { value: 'SERVO_SPINDLE_LOAD', label: 'Load' }, //GD_PLANT
    ...
    formattedHistoryData() {
      const parameter1Data = this.historyData.parameter1_data.map(item => ({
        time: new Date(item.time).getTime(),
        value: item.value,
        parameterName: item.parameter_name,
      }));

      const parameter2Data = this.historyData.parameter2_data.map(item => ({
        time: new Date(item.time).getTime(),
        value: item.value,
        parameterName: item.parameter_name,
      }));
  ...
  actions: {
    ...
    async fetchHistoryData(formData) {
      ...
      try {
        ...
        const fromDateEpoch = dayjs(fromDate).valueOf();
        const toDateEpoch = dayjs(toDate).valueOf();
        ...
        const response = await backendApi.get('/get-parameter-comparison-history/', {
          params: {
            ...
            parameter1,
            parameter2,
            from_datetime: fromDateEpoch/1000,
            to_datetime: toDateEpoch/1000,
          },
        });
